# PétiSign
PétiSign est une site de Pétitions en ligne. Il s'agit d'un projet annuel à l'ESGI.

## Mise en place (Setup)
Pour déployer PétiSign sur un VPS, la marche à suivre est relativement simple :
 1. Téléchargez le code source
 2. Créez les fichiers .env nécessaires (cf. `.gitignore`)
 3. Installez MariaDB sur votre VPS
 4. Exécutez le fichier `petisign_database.sql`
 5. Déployez les fichiers sur votre VPS
 6. Rendez vous sur `http://[votre_ip]/setup.php`
 7. Rentrez le mot de passe suivant : `3hIe1TuYoGUymZ8iePmwjVBniyGgYq8vmH0NM9i3PtAWKEULUS`

Une fois ces étapes effectuées, PétiSign devrait être déployé et prêt à l'emploi avec tout ce qu'il faut pour pouvoir le gérer via le BackOffice !
***N.B : n'oubliez pas de supprimer par la suite le fichier setup.php !***

## Contributions

[@LouisDetraux](https://github.com/Louiss1904)\
[@ThomasFauvart](https://github.com/ThomasFdev)
