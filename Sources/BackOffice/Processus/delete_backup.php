...
if($id_admin != 0){
    ...
    if ($real_path && is_file($real_path) && strpos($real_path, $archives_dir) === 0) {
        ...
        if ($deleted || !file_exists($real_path)) {
            header('Location: ../logs.php?status=success&msg=file_deleted');
            exit();
        } else {

            header('Location: ../logs.php?status=error&msg=permission_denied');
            exit();
        }
    } else {

        header('Location: ../logs.php?status=error&msg=invalid_file');
        exit();
    }
} else {
