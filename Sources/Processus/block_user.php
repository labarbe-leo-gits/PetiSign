...
try{
    ...
    $insert_stmt = $pdo->prepare("INSERT INTO BLOCKED_USER (id_user, id_blocked_user) VALUES (:user_id, :target_id)");
    $insert_stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $insert_stmt->bindParam(':target_id', $target_id, PDO::PARAM_INT);
    $insert_stmt->execute();
    echo "User blocked successfully.";

    echo "Removing friends";
    $remove_stmt = $pdo->prepare("DELETE FROM FRIEND WHERE (id_user = :user_id AND id_friend = :target_id) OR (id_user = :target_id AND id_friend = :user_id)");
    $remove_stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $remove_stmt->bindParam(':target_id', $target_id, PDO::PARAM_INT);
    $remove_stmt->execute();
